remove_specific_folders = function(path,
  ...
  remove_specific_folders___Single.Path = function(path,
                                                   recursive_folder=NULL,
                                                   files_list,
                                                   remove){
    #=============================================================================
  ...
  lapply(path, FUN=function(ith_path, ...){
    ...
    if(!is.null(recursive_folder) & length(path) == length(recursive_folder)){
      remove_specific_folders___Single.Path(ith_path, recursive_folder[ind], files_list, remove)
    }else if(is.null(recursive_folder)){
      remove_specific_folders___Single.Path(path = ith_path, files_list = files_list, remove = remove)
    }
